# https://docs.google.com/forms/u/1/d/e/1FAIpQLSeOjuo2IFHIBgJ3H7JeFS5tk6MdUK7__YcoI_fkSIBAeSkjog/formResponse?pli=1
import pyb
import machine, time
...
if y == 1:
    ...
    while pyb.millis() - x - (pyb.millis() - p) < (((((R*3.14)/2)/(2*rpm*r)))*60*1000):
        flag = 0
        while obj.distance_cm() < 5:                                             # turning the flag to 1 if it enters the loop once
            if flag == 0:
                p = pyb.millis()
                flag = 1
            led1.off()
